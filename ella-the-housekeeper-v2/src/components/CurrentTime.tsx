...
export default function RemainingTime({time, setTime}: {time: Date, setTime: Date}) {
    const timeout = Math.round((time.getTime()-setTime.getTime())/(1000*60));
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            setRemainingTime(prevRemainingTime => {
                if(Math.abs(prevRemainingTime) > 10 && Math.abs(timeout-prevRemainingTime)%5 === 0) {
